...
func (r *SQLRepository) Get(ctx context.Context, postID, userID int64) (Post, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT DISTINCT p.id,p.author_id,u.name,p.type,p.title,p.content,p.priority,p.expires_at,p.created_at,
		...
		FROM school_posts p JOIN users u ON u.id=p.author_id JOIN school_post_groups pg ON pg.post_id=p.id LEFT JOIN user_school_groups ug ON ug.group_id=pg.group_id WHERE p.id=$1 AND(p.author_id=$2 OR ug.user_id=$2 OR(SELECT role FROM users WHERE id=$2)IN('teacher','admin'))`, postID, userID).Scan(
