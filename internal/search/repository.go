...
func (r *SQLRepository) Search(ctx context.Context, userID int64, query string) ([]Result, error) {
	...
	rows, err := r.db.QueryContext(ctx, `
		SELECT type,id,title,excerpt,category,department,created_at,expires_at FROM (
		 ...
		 UNION ALL
		 SELECT 'contact',c.id,c.name,'質問カテゴリの担当窓口',c.name,g.name,c.created_at,NULL::timestamptz,c.created_at FROM question_categories c JOIN school_groups g ON g.id=c.group_id WHERE c.is_active AND (c.name ILIKE $2 OR g.name ILIKE $2)
	...
	for rows.Next() {
		var item Result
		if err := rows.Scan(&item.Type, &item.ID, &item.Title, &item.Excerpt, &item.Category, &item.Department, &item.CreatedAt, &item.ExpiresAt); err != nil {
			return nil, err
		}
